// ──────────────────────────────────────────────────────────
//  Página: Inicio de sesión (Login)
//  Pantalla de autenticación con formulario validado con Zod.
//  Muestra branding de bpA Motors y maneja errores de login.
// ──────────────────────────────────────────────────────────
...
// Esquema de validación estricto con Zod (Defense in Depth)
// La inyección SQL se previene en el backend, pero validar
// en frontend evita enviar payloads maliciosos obvios.
...
function LoginPage() {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    ...
    try {
      ...
      // Error manejado por AuthContext (loginError)
    } finally {
      ...
      <div className="flex w-full max-w-5xl overflow-hidden rounded-xl shadow-lg">
        ...
        <div className="hidden w-1/2 flex-col justify-between bg-[#03369A] p-12 text-white lg:flex">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-white/50">bPA Motors · Surquillo</p>
          </div>
        </div>
