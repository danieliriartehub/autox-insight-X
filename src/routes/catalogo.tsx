// ──────────────────────────────────────────────────────────
//  Página: Catálogo Maestro
//  Maestros de vehículos (por marca/modelo) y repuestos
//  con búsqueda y paginación desde la API backend.
// ──────────────────────────────────────────────────────────
...
function CatalogoPage() {
  ...
  // ── Estado para Repuestos (con debounce en búsqueda) ──
  ...
  // Fetch de marcas de vehículos desde la API
  const { data: brandsSummary, isLoading } = useQuery({
    queryKey: ["brands-summary"],
    queryFn: vehiclesApi.brandsSummary,
  });

  // Fetch de repuestos paginados desde la API
  const { data: partsResponse, isLoading: partsLoading } = useQuery({
    queryKey: ["parts", pageParts, debouncedQParts],
    queryFn: () =>
      partsApi.list({ page: pageParts, page_size: 10, search: debouncedQParts || undefined }),
  });

  // Procesa el resumen de marcas: agrupa modelos por marca y calcula totales
  const { marcas, modelosPorMarca } = useMemo(() => {
    if (!brandsSummary)
      return {
        marcas: [],
        modelosPorMarca: new Map<string, { nombre: string; cantidad: number }[]>(),
      };
  ...
  useEffect(() => {
    if (marcas.length > 0 && !selectedBrand) setSelectedBrand(marcas[0].marca);
  ...
  const PartsPaginationControls = () => (
    <div className="flex items-center gap-4">
      <span className="text-xs text-muted-foreground">
        {partsMetadata
          ? `Página ${partsMetadata.current_page} de ${partsMetadata.total_pages}`
          : "Página 1 de 1"}
      </span>
    ...
    <>
      ...
      <main className="flex-1 space-y-6 p-6">
        <Tabs defaultValue="veh" className="h-full">
          <TabsList>
            <TabsTrigger value="veh">
              <Car className="mr-1.5 h-4 w-4" /> Vehículos
            </TabsTrigger>
            <TabsTrigger value="rep">
              <Package className="mr-1.5 h-4 w-4" /> Repuestos
            </TabsTrigger>
          </TabsList>
                ...
                <div className="grid h-full grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x border-t">
                        ...
                        <ul className="divide-y">
                          {filteredMarcas.map((m) => (
                            <li
                              ...
                              className={`px-4 py-3 cursor-pointer hover:bg-muted/50 transition-colors flex justify-between items-center ${selectedBrand === m.marca ? "bg-primary/5 border-l-2 border-l-primary" : "border-l-2 border-l-transparent"}`}
                            >
                  ...
                  <div className="flex flex-col h-full md:col-span-2 overflow-hidden">
                    <div className="p-4 border-b bg-muted/5 flex justify-between items-center shrink-0">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {selectedModelos.length} modelos registrados
                      </span>
                    </div>
